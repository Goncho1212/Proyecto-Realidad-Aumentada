...
public class ARInteractionsManager : MonoBehaviour
{
    ...
    public GameObject Item3DModel //inicio de arpointer
    {
        set
        {
            item3DModel = value;
    ...
    void Start()  //configuración referencias iniciales
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touchOne.phase == TouchPhase.Moved)
            {
                if (aRRaycastManager.Raycast(touchOne.position, hits, TrackableType.Planes))
                {
                    ...
                    if (!isOverUI && isOver3DModel) //se presiona o el modelo o el espacio para mover
                    {
            ...
            if (Input.touchCount == 2)
            {
                ...
                {
                    Vector2 currentTouchPost = touchTwo.position - touchOne.position;
                    float angle = Vector2.SignedAngle(initialTouchPos, currentTouchPost);
                    item3DModel.transform.rotation = Quaternion.Euler(0, item3DModel.transform.eulerAngles.y - angle, 0);
                    initialTouchPos = currentTouchPost;
                }
            }
    ...
    public void SetItemPosition() //tap de model 3d
    {
        if (item3DModel != null)
        {
    ...
    public void DeleteItem()   //Eliminar modelo 3d
    {
        Destroy(item3DModel);
        aRPointer.SetActive(false);
